/**
 HeikinAshi_03
 - Still require 12/26 EMA alignment before allowing a directional bias, long or short.
 - Enter after green heikin ashi candle.
 - Don't exit position until the 12/26 EMA crosses the other way.
 - Set trailing stop under the 26 EMA by a few %?
...
function shouldTakeProfit(marketState, config, condition, offset=1) {
  ...
  const ema12 = imdTrend.ema12[offset]
  const ema26 = imdTrend.ema26[offset]
  if (condition === 'green' && (ema12 < ema26)) {
    return true
  }
  if (condition === 'red' && (ema12 > ema26)) {
    return true
  }
  return false
}
...
function trailingStopPrice(marketState, config, condition) {
  const imdTrend = marketState[`imd${config.trendTf}`]
  const ema26 = imdTrend.ema26[0]
  const factor = 0.005
  if (condition === 'green') {
    return ema26 - (factor * ema26)
  }
  if (condition === 'red') {
    return ema26 + (factor * ema26)
  }
